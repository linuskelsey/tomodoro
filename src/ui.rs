...
fn draw_label_input(f: &mut Frame, ls: &LabelState, area: Rect) {
    ...
    f.render_widget(
        Paragraph::new(line)
            .block(Block::default().borders(Borders::ALL).border_style(Style::default().fg(Color::DarkGray)).title(" task label ").title_bottom(Line::from(Span::styled(" Esc/Enter ", Style::default().fg(Color::Rgb(60, 60, 60)))).right_aligned())),
        popup,
    );
}
